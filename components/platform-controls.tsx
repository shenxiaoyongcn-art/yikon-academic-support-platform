...
type BmpSessionResponse = { authenticated?: boolean; identity?: BmpIdentity | null; contractVerified?: boolean; contractVersion?: string | null; error?: string };
...
export function BmpLoginControl() {
  ...
  useEffect(() => {
    ...
    void fetch('/api/bmp/session', { cache: 'no-store' })
      ...
      .then((result) => { if (!cancelled) { setIdentity(result.authenticated ? result.identity || null : null); setContractVerified(result.contractVerified === true); } })
      .catch(() => { if (!cancelled) { setIdentity(null); setContractVerified(false); } })
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage('BMP 身份验证成功；各模块数据接口仍需分别通过 IT 验收。');
      form.reset();
  ...
  return (
    <>
      <button type="button" className={`bmp-login-trigger${identity ? ' authenticated' : ''}`} onClick={() => setOpen(true)}>
        <span>{identity ? '✓' : '⇥'}</span>
        <b>{checking ? '检查身份' : identity ? `BMP · ${identity.name}` : contractVerified ? '验证BMP身份' : 'BMP身份待接入'}</b>
      </button>
      {open && (
        <div className="bmp-login-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <section className="bmp-login-dialog" role="dialog" aria-modal="true" aria-labelledby="bmp-login-title">
            ...
            <h2 id="bmp-login-title">{identity ? 'BMP 身份已验证' : '验证 BMP 账号'}</h2>
            ...
            {identity ? (
              <div className="bmp-account-card">
                ...
                <p>当前只代表身份验证成功，不代表科研、会议、销量或审批接口已经接通。</p>
              ...
              <form className="bmp-login-form" onSubmit={login}>
                ...
                <p className="bmp-password-note">姓名用于提交前核对，登录后的姓名和用户ID必须由BMP认证接口返回。密码仅用于本次验证，不写入平台数据库，也不会出现在导入导出文件中。</p>
                <button type="submit" disabled={saving}>{saving ? '正在验证…' : '验证 BMP 身份'}</button>
              </form>
            ) : (
              <div className="bmp-account-card">
                <span>待IT对接</span>
                <strong>BMP身份契约未验收</strong>
                <p>当前不收集BMP邮箱密码。请IT先确认SSO/OAuth或登录代理协议、返回字段、会话失效与退出机制，完成安全验收后才开放输入。</p>
              </div>
            )}
