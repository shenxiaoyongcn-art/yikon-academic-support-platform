...
import { demoActor, demoBudgets, demoContacts, demoData, demoPolicy, demoStakeholderInputs } from './demo.ts';
import { businessChain } from './chain.ts';
import { steps } from './definition.ts';
import { alerts, batchEligible, canAct, newCase, redact, transition, visible } from './workflow.ts';
import { cents, isDate, normalizeCase, type Case, type Command, type Data, type Role, type Route } from './model.ts';
...
function apply(c: Case, action: Command['action'] = 'advance', data = demoData(c.route), role: Role = steps[c.stage].role, extra: Partial<Command> = {}) {
  const command: Command = { action, data, expectedRevision: c.revision, ...extra };
  if (['draft', 'returned', 'reserve'].includes(c.stage) && command.stakeholders === undefined) command.stakeholders = demoStakeholderInputs();
  return transition(c, command, { actor: demoActor(role), now: now + c.revision * 1000, policy: demoPolicy, budget: demoBudgets[0], contacts: demoContacts, projectNo: `YKSR-${c.route}-TEST` });
}
...
function launch(route: Route = 'A') {
  ...
  if (route === 'C') {
    c = apply(c, 'submit_document', { kind: 'contract', title: '测试合同', reference: 'BMP-CONTRACT-TEST', evidence: '/test/contract', note: '合同已签订' }, 'pmo');
    c = apply(c, 'review_document', { documentId: c.documents[0].id, decision: 'verified' }, 'finance', { note: '已核对源系统编号和回执' });
  }
...
test('医院项目缺负责人或对接人不得提交', () => { assert.throws(() => apply(fresh(), 'advance', demoData(), 'applicant', { stakeholders: [] }), /项目负责人/); assert.throws(() => apply(fresh(), 'advance', demoData(), 'applicant', { stakeholders: demoStakeholderInputs().slice(0, 1) }), /日常对接人/); });
test('纯内部B类不强制医院联系人，关联医院后必须维护', () => { const data = { ...demoData('B'), customerId: '', hospital: '', clinicalDepartment: '', region: '' }; assert.equal(apply(fresh('B'), 'advance', data, 'applicant', { stakeholders: [] }).stage, 'intake'); assert.throws(() => apply(fresh('B'), 'advance', demoData('B'), 'applicant', { stakeholders: [] }), /项目负责人/); });
test('联系人须属于同一医院且重要程度有客观依据', () => { const other = { ...demoContacts[0], id: 'OTHER-CONTACT', customerId: 'OTHER' }; assert.throws(() => transition(fresh(), { action: 'advance', expectedRevision: 1, data: demoData(), stakeholders: [{ ...demoStakeholderInputs()[0], contactId: other.id }, demoStakeholderInputs()[1]] }, { actor: demoActor('applicant'), now, policy: demoPolicy, contacts: [other, ...demoContacts] }), /不匹配/); assert.throws(() => apply(fresh(), 'advance', demoData(), 'applicant', { stakeholders: demoStakeholderInputs().map((item, index) => index ? item : { ...item, importanceBasis: '' }) }), /责任和重要程度依据/); });
test('同一人可明确承担两个医院角色，但分别保存责任快照', () => { const inputs = demoStakeholderInputs().map(item => ({ ...item, contactId: demoContacts[0].id })); const c = apply(fresh(), 'advance', demoData(), 'applicant', { stakeholders: inputs }); assert.equal(c.stakeholders.length, 2); assert.equal(c.stakeholders[0].contactId, c.stakeholders[1].contactId); assert.notEqual(c.stakeholders[0].role, c.stakeholders[1].role); });
test('联系人主档更新不会反向改写已有审批快照', () => { const c = apply(fresh()); const oldName = c.stakeholders[0].profileSnapshot.name, updatedProfile = { ...demoContacts[0], name: '更新后的档案名称', revision: 2 }; assert.notEqual(updatedProfile.name, oldName); assert.equal(c.stakeholders[0].profileSnapshot.name, oldName); assert.equal(c.stakeholders[0].profileRevision, 1); });
test('旧项目缺少联系人数组时可兼容读取', () => { const old = { ...fresh() } as Case; delete (old as Partial<Case>).stakeholders; assert.deepEqual(normalizeCase(old).stakeholders, []); });
test('旧项目BMP对象缺少新回执字段时按未同步兼容读取', () => { const old = fresh(); old.bmp = { integrationStatus: 'synced', bmpProjectId: 'BMP-1', bmpWorkflowInstanceId: 'WF-1', bmpOfficialStatus: '已批准', officialDecision: 'approved', sourceVersion: 'v1' } as Case['bmp']; const normalized = normalizeCase(old); assert.equal(normalized.bmp.bmpProjectId, 'BMP-1'); assert.equal(normalized.bmp.budgetReceiptId, ''); assert.equal(normalized.bmp.settlementReceiptId, ''); });
...
test('BMP快照余额不足时不得越过预算校验', () => { const c = reach('marketing'); assert.throws(() => transition(c, { action: 'advance', data: demoData(), expectedRevision: c.revision }, { actor: demoActor('marketing'), now, policy: demoPolicy, budget: { ...demoBudgets[0], totalCents: 100 } }), /BMP快照可用额不足/); });
test('A类建档前必须形成完整成本预测基线', () => { const c = reach('commitment'); assert.equal(c.costForecastCents, 1200000); assert.equal(c.projectNo, null); assert.throws(() => apply({ ...c, costForecastCents: 0 }), /成本预测基线/); });
...
test('正式工作区无BMP已验收正式回执不得启动', () => { const c = reach('launch'); const actor = { ...demoActor('pmo'), demo: false }; assert.throws(() => transition(c, { action: 'advance', expectedRevision: c.revision, data: demoData(), milestones: c.milestones.map(m => ({ ...m, plannedDate: c.data.deadline, standard: '已确认' })) }, { actor, now, policy: demoPolicy, budget: demoBudgets[0] }), /BMP正式决策/); });
...
test('变更保留编号及成本预测，旧对象不可被覆盖', () => { const c = launch(), copy = JSON.stringify(c), next = apply(c, 'change', { sampleQuantity: '50' }, 'pmo', { note: '追加样本' }); assert.equal(JSON.stringify(c), copy); assert.equal(next.costForecastCents, c.costForecastCents); assert.equal(next.projectNo, c.projectNo); assert.equal(next.stage, 'intake'); assert.equal(next.decisions.length, 0); assert.equal(next.baseline, c.baseline); });
...
test('已执行项目终止后仍保留成本预测供复盘', () => { const c = launch(), next = apply(c, 'terminate', {}, 'pmo', { note: '客户取消' }); assert.equal(next.stage, 'settlement'); assert.equal(next.costForecastCents, c.costForecastCents); const done = apply(next, 'advance', demoData(), 'finance'); assert.equal(done.stage, 'terminated'); assert.equal(done.costForecastCents, c.costForecastCents); });
...
test('正式工作区不能用手工BMP财务字段绕过收口回执', () => { const c = apply(launch(), 'terminate', {}, 'pmo', { note: '终止' }); const actor = { ...demoActor('finance'), demo: false }; assert.throws(() => transition(c, { action: 'advance', expectedRevision: c.revision, data: { ...demoData(), bmpSettlementReceiptId: 'MANUAL', bmpSettlementStatus: '已完成' } }, { actor, now, policy: demoPolicy, budget: demoBudgets[0] }), /已验收接口回传/); });
test('项目收口不能在没有核销单时显示费用核销完成', () => { const c = apply(apply(launch(), 'terminate', {}, 'pmo', { note: '终止' }), 'advance', demoData(), 'finance'); const node = businessChain(c).find(item => item.key === 'expense'); assert.equal(node?.count, 0); assert.equal(node?.state, 'pending'); });
test('销售看不到内部成本和战略方案，包括单据金额', () => { const c = launch('B'); const actor = { ...demoActor('applicant'), demo: false }; c.creatorEmail = actor.email; c.documents.push({ id: 'x', title: '内部采购', status: 'verified', kind: 'payment', amount: '30000', applicant: 'finance@example.test', createdAt: now, reference: 'SECRET', evidence: '/internal', note: '成本30000' }); const safe = redact(c, actor); assert.equal(safe.data.testCost, undefined); assert.equal(safe.data.strategicGoal, undefined); assert.equal(safe.documents[0].amount, ''); assert.equal(safe.documents[0].reference, ''); });
...
test('关联付款只核验外部引用，不形成本地正式审批', () => { let c = launch(); c = apply(c, 'submit_document', { kind: 'payment', title: '外协付款引用', amount: '1000', reference: 'BMP-PAY-001', evidence: '/evidence', note: '用途' }, 'pmo'); const id = c.documents[0].id; assert.throws(() => apply(c, 'review_document', { documentId: id, decision: 'verified' }, 'pmo', { note: '已核对' }), /权限/); c = apply(c, 'review_document', { documentId: id, decision: 'verified' }, 'finance', { note: '已核对源系统编号和回执，不代表付款审批' }); assert.equal(c.documents[0].status, 'verified'); });
for (const route of ['A', 'B', 'C'] as Route[]) test(`${route}类全程可走到结题并归档`, () => {
  ...
  c = apply(c); assert.equal(c.stage, 'acceptance'); c = apply(c); assert.equal(c.stage, 'settlement'); c = apply(c); assert.equal(c.stage, 'closed'); assert.ok(c.costForecastCents >= 0);
